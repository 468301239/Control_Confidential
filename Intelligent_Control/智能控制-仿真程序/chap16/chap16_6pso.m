...
for i=1:1:Size
    Ji(i)=chap16_6obj(P(i,:),tol,Y,N);
    Pl(i,:)=P(i,:);
end
BestS=P(1,:); %全局最优个体
for i=2:Size
    if chap16_6obj(P(i,:),tol,Y,N)<chap16_6obj(BestS,tol,Y,N)
        BestS=P(i,:);
    end
end
%（3）进入主要循环，直到满足精度要求
...
%（4）判断和更新       
       if chap16_6obj(P(i,:),tol,Y,N)<Ji(i) %判断当此时的位置是否为最优的情况
          Ji(i)=chap16_6obj(P(i,:),tol,Y,N);
          Pl(i,:)=P(i,:);
        end
        if Ji(i)<chap16_6obj(BestS,tol,Y,N)
          BestS=Pl(i,:);
        end
    end
Best_J(kg)=chap16_6obj(BestS,tol,Y,N);
end
