...
BestS=P(1,:); %全局最优个体
for i=2:Size
    if chap16_8obj(P(i,:),tol,Y,N)<chap16_8obj(BestS,tol,Y,N)
        BestS=P(i,:);
    end
end
Ji=chap16_8obj(BestS,tol,Y,N);
%进入主要循环，直到满足精度要求
for kg=1:1:G
     time(kg)=kg;
%变异
...
%选择        
        if(chap16_8obj(v(i,:),tol,Y,N)<chap16_8obj(P(i,:),tol,Y,N))
            P(i,:)=v(i,:);
        end
%判断和更新       
       if chap16_8obj(P(i,:),tol,Y,N)<Ji %判断当此时的位置是否为最优的情况
          Ji=chap16_8obj(P(i,:),tol,Y,N);
          BestS=P(i,:);
        end
      
    end
Best_J(kg)=chap16_8obj(BestS,tol,Y,N);
end
