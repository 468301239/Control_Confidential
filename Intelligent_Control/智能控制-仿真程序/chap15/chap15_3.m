...
for i=1:Size
    p(i)=chap15_3func(x(i,:));
    y(i,:)=x(i,:);
    
    if i==1
        plocal(i,:)=chap15_3lbest(x(Size,:),x(i,:),x(i+1,:));
    elseif i==Size
        plocal(i,:)=chap15_3lbest(x(i-1,:),x(i,:),x(1,:));
        else 
        plocal(i,:)=chap15_3lbest(x(i-1,:),x(i,:),x(i+1,:));
    end
end

BestS=x(1,:);%初始化最优个体BestS
for i=2:Size
    if chap15_3func(x(i,:))>chap15_3func(BestS)
        BestS=x(i,:);
    end
end
...
%（4）判断和更新
       ...
       if chap15_3func(x(i,:))>p(i) %判断当此时的位置是否为最优的情况，当不满足时继续更新
          p(i)=chap15_3func(x(i,:));
          y(i,:)=x(i,:);
        end
        if p(i)>chap15_3func(BestS)
            BestS=y(i,:);
        end
    end
Best_value(kg)=chap15_3func(BestS);
end
figure(1);
kg=1:G;
